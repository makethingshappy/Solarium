...
#line 33 "lpc_chip_82x\\src\\spis_8xx.c"
...
static volatile _Bool deasserted;
...
uint32_t Chip_SPIS_XferHandler(LPC_SPI_T *pSPI, SPIS_XFER_T *xfer)
{
	...
	staterr = Chip_SPI_GetStatus(pSPI) & ((1 << 2) | (1 << 3));
	if (staterr != 0) {
	...
	while ((Chip_SPI_GetStatus(pSPI) & (1 << 1)) != 0) {
		if (xfer->txCount == 0) {
		...
		if ((xfer->txCount == 0) || (xfer->pTXData8 == ((void *) 0))) {
			data = 0;
		}
		else {
