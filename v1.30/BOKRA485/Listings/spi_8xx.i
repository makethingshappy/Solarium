...
#line 33 "lpc_chip_82x\\src\\spi_8xx.c"
...
uint32_t Chip_SPI_CalClkRateDivider(LPC_SPI_T *pSPI, uint32_t bitRate)
{
	...
	SPIClk = Chip_Clock_GetSystemClockRate();	 
...
void Chip_SPI_DelayConfig(LPC_SPI_T *pSPI, SPI_DELAY_CONFIG_T *pConfig)
{
	uint32_t delayValue = (((pConfig->PreDelay) & 0x0F) << 0) |
		(((pConfig->PostDelay) & 0x0F) << 4)                 |
		(((pConfig->FrameDelay) & 0x0F) << 8);
	if (pConfig->TransferDelay) {
		delayValue |= (((pConfig->TransferDelay - 1) & 0x0F) << 12);
	}
	pSPI->DLY = delayValue;
}
...
uint32_t Chip_SPI_RWFrames_Blocking(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
	...
	Chip_SPI_ClearStatus(pSPI, ((uint32_t) (1 << 2)) | ((uint32_t) (1 << 3)) | ((uint32_t) (1 << 4)) | ((uint32_t) (1 << 5)));
	Chip_SPI_SetControlInfo(pSPI, pXfSetup->DataSize, (0 << 16) | (1 << 21));
	...
	while ((pXfSetup->TxCnt < pXfSetup->Length) ||
		...
		if ((Status & (1 << 1)) && (pXfSetup->TxCnt < pXfSetup->Length)) {
			SPI_Send_Data(pSPI, pXfSetup);
		}

		 
		if ((Status & (1 << 0)) && (pXfSetup->RxCnt < pXfSetup->Length)) {
			SPI_Receive_Data(pSPI, pXfSetup);
		}
	}
	 
	if (Chip_SPI_GetStatus(pSPI) & (((uint32_t) (1 << 2)) | ((uint32_t) (1 << 3)))) {
		return 0;
	}
...
uint32_t Chip_SPI_WriteFrames_Blocking(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
	...
	Chip_SPI_ClearStatus(pSPI, ((uint32_t) (1 << 2)) | ((uint32_t) (1 << 3)) | ((uint32_t) (1 << 4)) | ((uint32_t) (1 << 5)));
	Chip_SPI_SetControlInfo(pSPI, pXfSetup->DataSize, (0 << 16) | (1 << 21) | (1 << 22));
	pXfSetup->TxCnt = pXfSetup->RxCnt = 0;
	while (pXfSetup->TxCnt < pXfSetup->Length) {
		 
		while (!(Chip_SPI_GetStatus(pSPI) & (1 << 1))) {}
	...
	while (!(Chip_SPI_GetStatus(pSPI) & (1 << 5))) {}
	Chip_SPI_ClearStatus(pSPI, ((uint32_t) (1 << 5)));

	 
	if (Chip_SPI_GetStatus(pSPI) & ((uint32_t) (1 << 3))) {
		return 0;
	}
	return pXfSetup->TxCnt;
}
...
uint32_t Chip_SPI_ReadFrames_Blocking(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
	...
	while (pXfSetup->RxCnt < pXfSetup->Length) {
		...
		while (!(Chip_SPI_GetStatus(pSPI) & (1 << 0))) {}
