...
#line 33 "lpc_chip_82x\\src\\irc_8xx.c"
...
{
	Chip_SYSCTL_PowerUp((1 << 1));									 
	Chip_Clock_SetSystemPLLSource(SYSCTL_PLLCLKSRC_IRC);						 
	Chip_Clock_SetMainClockSource(SYSCTL_MAINCLKSRC_IRC);
	Chip_FMC_SetFLASHAccess(FLASHTIM_30MHZ_CPU);								 
	Chip_SYSCTL_PowerDown((1 << 7));							 
	Chip_Clock_SetupSystemPLL(pll_cfg->msel, pll_cfg->psel);					 
	Chip_SYSCTL_PowerUp((1 << 7));								 
	while (!Chip_Clock_IsSystemPLLLocked()) {}									 
	Chip_Clock_SetSysClockDiv(pll_cfg->divider);								 
...
_Bool Chip_IRC_SetFreq(uint32_t main, uint32_t sys)
{
	uint16_t	freq_m = main/1000000;											 
	uint16_t	freq_s = sys/1000000;											 
	_Bool		found = 0;													 
	uint32_t	i = 0;
	...
	for (i=0; i<config_tab_ct; i++) {											 
		if ((freq_m == config_tab[i].freq_main) && (freq_s == config_tab[i].freq_sys)) {	 
			config_tab_idx = i;													 
			found = 1;														 
			break;																 
		}
	...
	if (found == 1) {														 
		pll_config(&config_tab[config_tab_idx]);								 
	}
...
void Chip_IRC_SetFreq_ROM(uint32_t sys)
{
	...
	cmd[0] = Chip_Clock_GetIntOscRate() / 1000;									 
	cmd[1] = sys / 1000;														 
	cmd[2] = 0;
	cmd[3] = sys / 10000;														 
	(((*(LPC_ROM_API_T * *) 0x1FFF1FF8UL))->pPWRD)->set_pll(cmd, resp);											 
