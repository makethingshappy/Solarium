#line 1 "lpc_chip_82x\\src\\pinint_8xx.c"
...
#line 33 "lpc_chip_82x\\src\\pinint_8xx.c"
...
void Chip_PININT_SetPatternMatchSrc(LPC_PIN_INT_T *pPININT, uint8_t chan, Chip_PININT_BITSLICE_T slice)
{
    ...
    pmsrc_reg = pPININT->PMSRC & ~((7 << (8 + (slice * 3)))
		| 0xff);
	pPININT->PMSRC = pmsrc_reg | (chan << (8 + (slice * 3)));
}
        ...
        Chip_PININT_BITSLICE_CFG_T slice_cfg, _Bool end_point)
{
    ...
    pmcfg_reg = pPININT->PMCFG & ~((7 << (8 + (slice * 3)))
		| (1<<7));
    pPININT->PMCFG = pmcfg_reg | (slice_cfg << (8 + (slice * 3)));
	...
	if (end_point == 1)
	{
        ...
        {
			pPININT->PMCFG = (0x1 << slice) | (pPININT->PMCFG & ~(1<<7));
		}
	}
}
