...
#line 33 "lpc_chip_82x\\src\\spim_8xx.c"



 

static volatile _Bool xmitOn;
...
uint32_t Chip_SPIM_GetClockRate(LPC_SPI_T *pSPI)
{
	return Chip_Clock_GetSystemClockRate() / ((pSPI->DIV & ~0xffff0000) + 1);
}
...
void Chip_SPIM_DelayConfig(LPC_SPI_T *pSPI, SPIM_DELAY_CONFIG_T *pConfig)
{
	pSPI->DLY = ((((pConfig->PreDelay) & 0x0F) << 0) |
				 (((pConfig->PostDelay) & 0x0F) << 4) |
				 (((pConfig->FrameDelay) & 0x0F) << 8) |
				 (((pConfig->TransferDelay - 1) & 0x0F) << 12));
}
...
void Chip_SPIM_XferHandler(LPC_SPI_T *pSPI, SPIM_XFER_T *xfer)
{
	...
	flen = ((pSPI->TXCTRL & ~(0xffff|(7<<17)|(1<<23)|(0xfu<<28))) >> 24) & 0xF;

	 
	if ((Chip_SPI_GetStatus(pSPI) & (1 << 4)) != 0) {
		Chip_SPI_ClearStatus(pSPI, (1 << 4));
	...
	if ((Chip_SPI_GetStatus(pSPI) & (1 << 5)) != 0) {
		Chip_SPI_ClearStatus(pSPI, (1 << 5));
	...
	while (((Chip_SPI_GetStatus(pSPI) & (1 << 1)) != 0) && (xmitOn == 1)) {
	...
	while ((Chip_SPI_GetStatus(pSPI) & (1 << 0)) != 0) {
		 
		data = Chip_SPI_ReadRawRXFifo(pSPI);

		 
		if (xfer->pRXData8 != ((void *) 0)) {
...
void Chip_SPIM_Xfer(LPC_SPI_T *pSPI, SPIM_XFER_T *xfer)
{
	...
	pSPI->TXCTRL = xfer->options | (0xF << 16);
